...
def read_t2t_examples(data_path):
    ...
    with open(data_path, "r") as f:
        for line in tqdm(f, desc=f"reading from {data_path}"):
            example = json.loads(line.strip())
            source_texts.append(example["text"]) # the </s> will be added after batch padding
            target_texts.append(example["label"]+" </s>")
            # </s>  is added here because the target sequence usually does not exceed the length limit
    return source_texts, target_texts


def convert_t2t_examples(data_path, tokenizer, max_src_length, max_tgt_length,append_token=" </s>"):
